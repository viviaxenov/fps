...
def jax_log_prob(x: jnp.ndarray) -> jnp.ndarray:
    ...
    norm_half = jnp.sum(jnp.abs(x - m) ** 0.5) ** 2.0
    return -norm_half
...
plt.figure()
plt.plot(hs_sorted, lasts_sorted, marker="o", label="last")
plt.plot(hs_sorted, bests_sorted, marker="o", label="best (min)")
plt.xlabel("h")
plt.ylabel("sinkdiv")
plt.legend()
plt.tight_layout()
out_both = out_h5.replace(".h5", "_last_and_best_sinkdiv_vs_h.png")
plt.savefig(out_both, dpi=300, bbox_inches="tight")
plt.close()
print("saved plot:", out_both)
